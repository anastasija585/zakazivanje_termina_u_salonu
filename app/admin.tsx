...
import { getData, patchData, deleteData } from "../firebase/firebaseApi";
...
export default function AdminScreen() {
  ...
  const changeStatus = async (id: string, status: string) => {
    await patchData(`reservations/${id}`, { status });
  ...
  const saveEdit = async () => {
    ...
    await patchData(`reservations/${editingId}`, {
      date: editDate,
      time: editTime,
      status: editStatus,
    });
...
const styles = StyleSheet.create({
  ...
  smallButtonOutlineText: { color: "#8a5f52", fontWeight: "700" },
  title: {
    fontSize: 34,
    fontWeight: "700",
    color: "#6b3f35",
  ...
  text: { fontSize: 15, color: "#6d4c41", marginBottom: 5 },
  status: {
    fontSize: 16,
    fontWeight: "700",
    color: "#4e342e",
  ...
  actionButtonLightText: { color: "#8a5f52", fontWeight: "700" },
  ...
  outlineButtonText: { color: "#8a5f52", fontWeight: "700" },
